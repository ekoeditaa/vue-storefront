<template>
  <div id="home">
    <main-slider />
    <promoted-offers/>
    <collection :title="$t('Bestsellers')" category="Women"/>
    <section class="new-collection container px15">
      <div>
        <header class="col-md-12">
          <h2 class="align-center h1 cl-secondary">{{ $t('The Macallan Trilogy ') }}</h2>
        </header>
      </div>
      <product-listing columns="3" :only-image="true" :products="everythingNewCollection" />
    </section>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    everythingNewCollection () {
      return this.$store.state.homepage.new_collection.slice(0, 3)
    },
    coolBagsCollection () {
